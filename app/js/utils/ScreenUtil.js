...
export function dealListTime(current: String) {
    // console.log('time-', new Date(Date.parse(current)))
    try {

        // console.log('----', toDate(new Date().getTime(), 'yyyy-MM-dd'))
        let now = new Date().getTime() / 1000
        let nowstr = now / 3600 / 24

        // let date = new Date(Date.parse(current))

        let date = strTimeToNum(current)


        let commoninStr = date.getTime() / 1000 / 3600 / 24
        ...
        // console.log('commoninStr', commoninStr)
        if (parseInt(nowstr - commoninStr) == 0) {
            return h + ":" + minute + ":" + second
        } else {
            if (parseInt(nowstr - commoninStr) == 1) {
                return "昨天 " + h + ":" + minute + ":" + second
            } else if (parseInt(nowstr - commoninStr) == 2) {
...
export function strTimeToNum(timeStr: String) {
    let dateParam = timeStr.split(/[\s-:]/)
